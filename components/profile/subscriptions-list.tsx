...
export function SubscriptionsList({ subscriptions }: { subscriptions: Subscription[] }) {
  ...
  async function handleCancel(id: string) {
    if (!confirm("Biztosan le szeretnéd mondani ezt az előfizetést?")) return;
    setLoading(id);
    setError(null);
    try {
  ...
  if (subscriptions.length === 0) {
    return (
      <p className="text-sm text-gray-500">Nincs aktív előfizetésed.</p>
    );
  }
        ...
        >
          <div className="text-sm">
            <p className="font-medium text-gray-800">
              {sub.tier.shelter?.name ?? "Ismeretlen menhely"} – {sub.tier.name}
            </p>
            <p className="text-gray-500">
              {sub.tier.amount.toLocaleString("hu-HU")} Ft / hó
            </p>
            <p className="mt-0.5 text-xs text-gray-400">
              Feliratkozva: {new Date(sub.createdAt).toLocaleDateString("hu-HU")}
            </p>
          </div>
