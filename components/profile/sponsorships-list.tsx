...
export function SponsorshipsList({ sponsorships }: { sponsorships: Sponsorship[] }) {
  ...
  async function handleCancel(id: string) {
    if (!confirm("Biztosan lemondod a virtuális örökbefogadást?")) return;
  ...
  if (sponsorships.length === 0) {
    return <p className="text-sm text-gray-500">Még nincs virtuális örökbefogadásod.</p>;
  }
        ...
        <div key={s.id} className="flex items-center justify-between rounded-xl border border-gray-100 bg-gray-50 px-4 py-3">
          <div className="text-sm">
            <p className="flex items-center gap-1.5 font-medium text-gray-800">
              ...
              {s.animal?.name ?? "Ismeretlen állat"}
            </p>
            <p className="text-gray-500">{s.amount.toLocaleString("hu-HU")} Ft / hó</p>
            <p className="mt-0.5 text-xs text-gray-400">
              {s.isPublic ? "Nyilvános virtuális gazdi" : "Anonim támogató"} ·{" "}
              {new Date(s.createdAt).toLocaleDateString("hu-HU")} óta
            </p>
          </div>
          <div className="flex items-center gap-2">
            {s.status === "CANCELLED" ? (
              <span className="rounded-full bg-gray-200 px-2.5 py-0.5 text-xs text-gray-500">Lemondva</span>
            ) : (
              <>
                <span className="rounded-full bg-green-100 px-2.5 py-0.5 text-xs font-medium text-green-700">Aktív</span>
