  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-gray-600">
        Érdeklődsz <span className="font-semibold">{animalName}</span> örökbefogadása iránt?
        Küldj üzenetet a menhelynek és ők hamarosan visszajeleznek!
      </p>

      {/* Shelter contact info */}
      {(shelter.phone || shelter.email) && (
        <div className="rounded-xl bg-gray-50 border border-gray-100 p-4 space-y-2">
          <p className="text-xs font-semibold uppercase tracking-wide text-gray-400">Menhely elérhetősége</p>
          {shelter.phone && (
            <a
      ...
      >
        ...
        {loading ? "Megnyitás..." : "Üzenet küldése a menhelynek"}
      </button>
    </div>
  );
}
