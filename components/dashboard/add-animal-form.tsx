...
const schema = z.object({
  ...
  imageUrl:       z.string().url("Érvénytelen URL").optional().or(z.literal("")),
});
...
export function AddAnimalForm({ onClose }: { onClose: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      {/* Kép URL */}
      <div>
        <h3 className="mb-3 text-xs font-semibold uppercase tracking-wide text-gray-400">Fotó</h3>
        <Field label="Kép URL" error={errors.imageUrl?.message}>
          <input {...register("imageUrl")} type="url" placeholder="https://..." className={cls} />
          <p className="mt-1 text-xs text-gray-400">
            Tölts fel egy képet pl. <a href="https://imgbb.com" target="_blank" rel="noopener noreferrer" className="text-brand-500 hover:underline">imgbb.com</a>-ra és illeszd be a linket.
          </p>
        </Field>
      </div>
