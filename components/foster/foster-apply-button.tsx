  ...
  if (existingStatus) {
    return (
      <div className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-medium ${FOSTER_STATUS_COLORS[existingStatus]}`}>
        ...
        Ideiglenes befogadó: {FOSTER_STATUS_LABELS[existingStatus]}
      </div>
    );
  }

  function toggleType(t: AnimalType) {
    ...
    <div>
      {!open ? (
        ...
        <form onSubmit={handleSubmit} className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm space-y-4">
          ...
          <div>
            <label className="mb-1.5 block text-xs font-semibold text-gray-600">Milyen állatot fogadnál? (opcionális)</label>
            <div className="flex flex-wrap gap-2">
              {ALL_ANIMAL_TYPES.map(t => (
          ...
          <div>
            <label className="mb-1 block text-xs font-semibold text-gray-600">Maximális súly (kg, opcionális)</label>
          ...
          <label className="flex items-center gap-2 text-sm text-gray-700">
            ...
            Tudok karantént / elkülönítést biztosítani
          </label>

          <div>
            <label className="mb-1 block text-xs font-semibold text-gray-600">Motiváció (opcionális)</label>
            <textarea value={motivation} onChange={e => setMotivation(e.target.value)}
              rows={3} maxLength={2000} placeholder="Miért szeretnél ideiglenes befogadó lenni?"
