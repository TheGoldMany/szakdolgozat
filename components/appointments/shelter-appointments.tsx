    ...
    <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
      <div className="flex items-start gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-2">
            <span className={`rounded-full border px-2.5 py-0.5 text-xs font-semibold ${STATUS_COLOR[appt.status]}`}>
              {t(STATUS_T_KEY[appt.status] as Parameters<typeof t>[0])}
            </span>
          ...
          {appt.note && (
            <p className="mt-2 rounded-lg bg-gray-50 px-3 py-2 text-xs text-gray-600">
              <span className="font-medium">{t("messageLabel")}</span> {appt.note}
            </p>
          )}
          {appt.adminNote && (
            <p className="mt-1.5 text-xs text-gray-400">
              {t("adminNote")} {appt.adminNote}
            </p>
          )}
        </div>
        ...
        {appt.status === "CONFIRMED" && (
          <button onClick={() => act("COMPLETE")} disabled={loading}
            ...
            <CheckCheck className="h-3.5 w-3.5" /> {t("completeBtn")}
          </button>
        )}
      </div>
      ...
      {expanded && appt.status === "PENDING" && (
        <div className="mt-4 border-t border-gray-100 pt-4 space-y-3">
          <div>
            <label className="mb-1 block text-xs font-semibold text-gray-600">{t("confirmTime")}</label>
          ...
          <div>
            <label className="mb-1 block text-xs font-semibold text-gray-600">{t("replyToApplicant")}</label>
            <textarea value={adminNote} onChange={e => setAdminNote(e.target.value)}
              rows={2} maxLength={500} placeholder={t("replyPlaceholder")}
          ...
          <div className="flex gap-2">
            <button onClick={() => act("CONFIRM", { confirmedAt: new Date(confirmedAt).toISOString() })}
              ...
              <Check className="h-4 w-4" /> {t("confirmBtn")}
            </button>
            <button onClick={() => act("REJECT")} disabled={loading}
              ...
              <X className="h-4 w-4" /> {t("rejectBtn")}
            </button>
          </div>
        </div>
      )}
  ...
  return (
    <div>
      ...
      <div className="mb-5 flex flex-wrap gap-2">
        {TABS.map(tabKey => {
          const count = initial.filter(a => a.status === tabKey).length;
          return (
            <button key={tabKey} onClick={() => setTab(tabKey)}
              ...
              {t(STATUS_T_KEY[tabKey] as Parameters<typeof t>[0])} {count > 0 && <span className="ml-1 rounded-full bg-white/20 px-1.5 py-0.5 text-xs">{count}</span>}
            </button>
          );
        })}
      </div>
      ...
      {filtered.length === 0 ? (
        <p className="rounded-2xl border border-dashed border-gray-200 py-10 text-center text-sm text-gray-400">
          {t("noAppt", { tab: t(STATUS_T_KEY[tab] as Parameters<typeof t>[0]).toLowerCase() })}
        </p>
      ) : (
