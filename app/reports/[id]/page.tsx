    ...
    <div className="min-h-screen bg-gray-50">
      <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div className="rounded-2xl border border-gray-100 bg-white shadow-sm overflow-hidden">
          {/* Fejléc sáv */}
          <div className={cn("px-6 py-4 flex items-center justify-between gap-3",
            report.type === "LOST" ? "bg-red-50" : report.type === "FOUND" ? "bg-green-50" : "bg-yellow-50"
          )}>
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              {report.color  && <div><span className="text-gray-400">Szín:</span> <span className="font-medium">{report.color}</span></div>}
              {report.gender && <div><span className="text-gray-400">Nem:</span> <span className="font-medium">{report.gender === "MALE" ? "Hím" : report.gender === "FEMALE" ? "Nőstény" : "Ismeretlen"}</span></div>}
            </div>
            ...
            <div className="rounded-xl bg-gray-50 p-4 space-y-2">
              <p className="text-sm font-semibold text-gray-700">Kapcsolattartó: {report.contactName}</p>
              {report.contactPhone && (
                <a href={`tel:${report.contactPhone}`} className="flex items-center gap-2 text-sm text-brand-500 hover:underline">
                  <Phone className="h-4 w-4" />{report.contactPhone}
                </a>
              )}
              <a href={`mailto:${report.contactEmail}`} className="flex items-center gap-2 text-sm text-brand-500 hover:underline">
